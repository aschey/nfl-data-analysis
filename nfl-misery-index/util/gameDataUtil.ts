...
export const getMatchups = async (
  ...
  const games = await getJson<Game[]>(`/games?weekId=${weekId}`);
...
export const getWeeks = async (year: number): Promise<Value<number>[]> => {
  const weeks = await getJson<Week[]>(`/weeks?year=${year}`);
...
export const getScores = (weekId: number): Promise<Score[]> =>
  getJson<Score[]>(`/scores?weekId=${weekId}`);
