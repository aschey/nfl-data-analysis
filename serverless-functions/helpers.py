import json
from pysqlite3 import Cursor, connect, PARSE_DECLTYPES
from flask import request
from typing import Dict, List, Tuple, Any, Type, TypeVar, Optional, cast
...
def _dict_factory(cursor: Cursor, row: List[str]) -> Dict[str, str]:
    d = {
        _camel(col[0]): _convert_datatype(col[0], row[idx])
...
def _get_team(
    team_key: str, result: Dict[str, str], fields: List[str]
) -> Dict[str, str]:
    return {
        field: result[f"{team_key}{field[0].upper()}{field[1:]}"] for field in fields
    }
...
def get_team_structure(
    result: List[Dict[str, Any]], fields: List[str]
